...
import { Shirt, Star, Heart, Users, Zap, Award, Clock, MapPin } from 'lucide-react'

export default function GomlekPage() {
  ...
  useEffect(() => {
    ...
    const foundSubcategory = foundCategory.subcategories?.find((sub) => sub.slug === 'erkek')
    if (!foundSubcategory) return
    setSubcategory(foundSubcategory)

    const foundSubsubcategory = foundSubcategory.subcategories?.find((subsub) => subsub.slug === 'gomlek')
    ...
    const mapped = rawListings
      .filter(listing => 
        ...
        (listing.subCategory?.toLowerCase() ?? '') === 'erkek' &&
        listing.subSubCategory?.toLowerCase() === 'gomlek'
      )
        ...
        <aside className="w-64">
          <div className="bg-white rounded-lg shadow-md p-6">
            <h2 className="text-lg font-semibold mb-4 flex items-center">
              <Shirt className="w-5 h-5 text-blue-500 mr-2" />
              Erkek Giyim
            </h2>
                ...
                <li key={subsub.slug}>
                  <Link 
                    ...
                    className={`flex items-center text-gray-700 hover:text-blue-600 transition-colors ${
                      subsub.slug === 'gomlek' ? 'text-blue-600 font-semibold' : ''
                    }`}
                  >
            ...
            <div className="mt-8">
              <h3 className="font-semibold mb-3">Gömlek Özellikleri</h3>
        ...
        <main className="flex-1">
          <div className="mb-8">
            ...
            <p className="text-gray-600">
              Şık ve kaliteli erkek gömlek modelleri ile tarzınızı yansıtın. Resmi ve günlük kullanım için uygun gömlekler.
            </p>
          </div>

          {/* Featured Ads */}
          <div className="mb-8">
            <h2 className="text-xl font-semibold mb-4">Öne Çıkan Gömlek Ürünleri</h2>
            <FeaturedAds 
          ...
          {/* Latest Ads */}
          <div>
            <h2 className="text-xl font-semibold mb-4">En Yeni Gömlek Ürünleri</h2>
            <LatestAds 
              category={category.slug} 
              subcategory={subcategory.slug} 
              listings={mappedListings} 
