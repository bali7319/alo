...
export default function YaziciCategoryPage() {
  ...
  const yaziciListings: Listing[] = listings
    .filter(listing => 
      listing.category === 'elektronik' && 
      listing.subCategory === 'yazici'
    )

  // Yazıcı ilanlarını filtrele ve types/listings.ts formatına dönüştür
  const filteredListings: Listing[] = yaziciListings.filter(listing => {
