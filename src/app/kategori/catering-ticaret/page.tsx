// catering-ticaret klasörü dinamik route'u bloke ediyor
...
export async function generateMetadata(): Promise<Metadata> {
  const foundCategory = categories.find((cat) => cat.slug === 'catering-ticaret');
  if (!foundCategory) return { title: 'Kategori Bulunamadı' };
  const categoryName = 'Catering & Ticaret';
  let listingCount = 0;
  try {
    listingCount = await withTimeout(
      prisma.listing.count({
        where: {
          OR: [{ category: 'catering-ticaret' }, { category: 'Catering & Ticaret' }],
          isActive: true,
  ...
  return {
    ...
    openGraph: {
      title,
      description,
      url: `https://alo17.tr/kategori/catering-ticaret`,
      siteName: 'Alo17',
      locale: 'tr_TR',
      type: 'website',
    },
    ...
    alternates: {
      canonical: `https://alo17.tr/kategori/catering-ticaret`,
    },
  };
}

export default async function CateringTicaretPage() {
  const slug = 'catering-ticaret';
