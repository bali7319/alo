...
export default function CateringTicaretPage() {
  const [isLoading, setIsLoading] = useState(true)
  const [category, setCategory] = useState<any>(null)
  const [mappedListings, setMappedListings] = useState<Listing[]>([])
  
  useEffect(() => {
    // Catering & Ticaret kategorisini bul
    ...
    if (foundCategory) {
      ...
      const mapped = rawListings
        .filter(listing => 
          listing.category.toLowerCase() === 'catering & ticaret' || 
          listing.category.toLowerCase() === 'catering-ticaret'
        )
  ...
  const subcategoryColors: { [key: string]: string } = {
    ...
    'ticaret': 'text-blue-500',
    'toplu-yemek': 'text-green-500',
    'diger': 'text-gray-500'
  }
          ...
          <ol className="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li aria-current="page">
              <div className="flex items-center">
                ...
                <span className="text-gray-500">Catering & Ticaret</span>
              </div>
            </li>
          </ol>
        </nav>
              ...
              <ul className="space-y-2">
                {category.subcategories?.map((subcategory: any) => (
                  <li key={subcategory.slug}>
                    <Link 
                      href={`/kategori/catering-ticaret/${subcategory.slug}`}
