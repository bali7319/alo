...
import { Refrigerator, Snowflake, Zap, Thermometer } from 'lucide-react'

export default function BuzdolabiPage() {
  ...
  // Buzdolabı ilanlarını filtrele
  const buzdolabiListings: Listing[] = rawListings
    .filter(listing => 
      listing.category.toLowerCase() === 'ev-ve-bahce' && 
      (listing.subCategory?.toLowerCase() ?? '') === 'beyaz-esya' &&
      listing.subSubCategory?.toLowerCase() === 'buzdolabi'
    )
  ...
  useEffect(() => {
    ...
    const foundSubSubcategory = foundSubcategory.subcategories?.find((subsub) => subsub.slug === 'buzdolabi')
    if (!foundSubSubcategory) return
    setSubSubcategory(foundSubSubcategory)
    
    setMappedListings(buzdolabiListings)
  }, [])
      ...
      <div className="flex gap-8">
        ...
        <aside className="w-64">
          <div className="bg-white rounded-lg shadow-md p-6">
            ...
            <div className="mt-8">
              <h3 className="font-semibold mb-3">Buzdolabı Özellikleri</h3>
        ...
        <main className="flex-1">
          <div className="mb-8">
            ...
            <p className="text-gray-600">
              No-Frost teknolojili, enerji tasarruflu buzdolapları ve dondurucular
            </p>
          </div>

          {/* Featured Ads */}
          <div className="mb-8">
            <h2 className="text-xl font-semibold mb-4">Öne Çıkan Buzdolabı İlanları</h2>
            <FeaturedAds 
          ...
          {/* Latest Ads */}
          <div>
            <h2 className="text-xl font-semibold mb-4">En Yeni Buzdolabı İlanları</h2>
            <LatestAds 
              category={category.slug} 
              subcategory={subcategory.slug} 
              listings={mappedListings} 
