...
export default function SehpaPage() {
  ...
  useEffect(() => {
    ...
    const foundSubSubcategory = foundSubcategory.subcategories?.find((subsub) => subsub.slug === 'sehpa')
    ...
    const mapped = rawListings
      .filter(listing => 
        ...
        listing.subSubCategory?.toLowerCase() === 'sehpa'
      )
                  ...
                  <li key={subsub.slug}>
                    <Link 
                      ...
                      className={`flex items-center space-x-3 p-2 rounded-lg hover:bg-gray-50 transition-colors ${
                        subsub.slug === 'sehpa' ? 'bg-emerald-50 text-emerald-700' : ''
                      }`}
                    >
