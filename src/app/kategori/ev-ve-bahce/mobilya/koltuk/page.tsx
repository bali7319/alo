...
export default function KoltukPage() {
  ...
  useEffect(() => {
    ...
    const foundSubSubcategory = foundSubcategory.subcategories?.find((subsub) => subsub.slug === 'koltuk')
    ...
    const mapped = rawListings
      .filter(listing => 
        ...
        listing.subSubCategory?.toLowerCase() === 'koltuk'
      )
                  ...
                  <li key={subsub.slug}>
                    <Link 
                      ...
                      className={`flex items-center space-x-3 p-2 rounded-lg hover:bg-gray-50 transition-colors ${
                        subsub.slug === 'koltuk' ? 'bg-amber-50 text-amber-700' : ''
                      }`}
                    >
