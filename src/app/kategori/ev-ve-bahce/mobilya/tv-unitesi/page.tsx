...
export default function TvUnitesiPage() {
  ...
  useEffect(() => {
    ...
    const foundSubSubcategory = foundSubcategory.subcategories?.find((subsub) => subsub.slug === 'tv-unitesi')
    ...
    const mapped = rawListings
      .filter(listing => 
        ...
        listing.subSubCategory?.toLowerCase() === 'tv-unitesi'
      )
                  ...
                  <li key={subsub.slug}>
                    <Link 
                      ...
                      className={`flex items-center space-x-3 p-2 rounded-lg hover:bg-gray-50 transition-colors ${
                        subsub.slug === 'tv-unitesi' ? 'bg-yellow-50 text-yellow-700' : ''
                      }`}
                    >
