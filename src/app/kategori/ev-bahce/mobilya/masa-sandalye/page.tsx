...
const subcategories = [
  { id: 'yemek-masasi', name: 'Yemek Masası', icon: <FaTable className="inline mr-2 text-amber-500" /> },
  { id: 'sandalye', name: 'Sandalye', icon: <FaChair className="inline mr-2 text-amber-500" /> },
  { id: 'masa-takimi', name: 'Masa Takımı', icon: <FaUtensils className="inline mr-2 text-amber-500" /> },
  { id: 'bar-stool', name: 'Bar Stool', icon: <FaChair className="inline mr-2 text-amber-500" /> },
]

export default function MasaSandalyeCategoryPage() {
  const [selectedSubcategory, setSelectedSubcategory] = useState<string | null>(null)
  const [priceRange, setPriceRange] = useState<string | null>(null)
  const [condition, setCondition] = useState<string | null>(null)

  // Masa sandalye ilanlarını filtrele
  const masaSandalyeListings = listings.filter(listing =>
    listing.category === 'ev-bahce' &&
    listing.subCategory === 'mobilya'
  )

  // Filtreleme fonksiyonu
  const filteredListings = masaSandalyeListings.filter(listing => {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900">Masa Sandalye</h1>
        <p className="text-gray-600 mt-2">
          Yemek masası, sandalye, masa takımı ve diğer yemek odası mobilyaları
        </p>
      </div>
