...
const subcategories = [
  { id: 'koltuk-takimi', name: 'Koltuk Takımı', icon: <FaCouch className="inline mr-2 text-brown-500" /> },
  { id: 'kanape', name: 'Kanape', icon: <FaCouch className="inline mr-2 text-brown-500" /> },
  { id: 'koltuk', name: 'Tek Koltuk', icon: <FaChair className="inline mr-2 text-brown-500" /> },
  { id: 'berjer', name: 'Berjer', icon: <FaChair className="inline mr-2 text-brown-500" /> },
]

export default function KoltukCategoryPage() {
  const [selectedSubcategory, setSelectedSubcategory] = useState<string | null>(null)
  const [priceRange, setPriceRange] = useState<string | null>(null)
  const [condition, setCondition] = useState<string | null>(null)

  // Koltuk ilanlarını filtrele
  const koltukListings = listings.filter(listing =>
    listing.category === 'ev-bahce' &&
    listing.subCategory === 'mobilya'
  )

  // Filtreleme fonksiyonu
  const filteredListings = koltukListings.filter(listing => {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900">Koltuk</h1>
        <p className="text-gray-600 mt-2">
          Koltuk takımı, kanape, tek koltuk ve berjer ürünleri
        </p>
      </div>
