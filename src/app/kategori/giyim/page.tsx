...
export default function GiyimCategoryPage() {
  ...
  useEffect(() => {
    ...
    const foundCategory = categories.find(cat => cat.slug === 'giyim')
    ...
    if (foundCategory) {
      ...
      const mapped = rawListings
        .filter(listing => listing.category.toLowerCase() === 'giyim')
  ...
  const subcategoryColors: { [key: string]: string } = {
    'bayan-giyim': 'text-pink-500',
    'erkek-giyim': 'text-blue-500',
    'cocuk-giyim': 'text-purple-500',
    'ayakkabi-canta': 'text-orange-500',
    'aksesuar': 'text-yellow-500',
    'diger': 'text-gray-500'
  }
          ...
          <ol className="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li aria-current="page">
              <div className="flex items-center">
                ...
                <span className="text-gray-500">Giyim</span>
              </div>
            </li>
          </ol>
        </nav>
              ...
              <ul className="space-y-2">
                {category.subcategories?.map((subcategory: any) => (
                  <li key={subcategory.slug}>
                    <Link 
                      href={`/kategori/giyim/${subcategory.slug}`}
