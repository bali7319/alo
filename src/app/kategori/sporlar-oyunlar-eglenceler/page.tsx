// sporlar-oyunlar-eglenceler klasörü dinamik route'u bloke ediyor
...
export async function generateMetadata(): Promise<Metadata> {
  const foundCategory = categories.find((cat) => cat.slug === 'sporlar-oyunlar-eglenceler');
  if (!foundCategory) return { title: 'Kategori Bulunamadı' };
  const categoryName = 'Sporlar, Oyunlar ve Eğlenceler';
  let listingCount = 0;
  try {
    listingCount = await withTimeout(
      prisma.listing.count({
        where: {
          OR: [{ category: 'sporlar-oyunlar-eglenceler' }, { category: 'Sporlar, Oyunlar ve Eğlenceler' }],
          isActive: true,
  ...
  return {
    ...
    openGraph: {
      title,
      description,
      url: `https://alo17.tr/kategori/sporlar-oyunlar-eglenceler`,
      siteName: 'Alo17',
      locale: 'tr_TR',
      type: 'website',
    },
    ...
    alternates: {
      canonical: `https://alo17.tr/kategori/sporlar-oyunlar-eglenceler`,
    },
  };
}

export default async function SporlarOyunlarEglencelerPage() {
  const slug = 'sporlar-oyunlar-eglenceler';
