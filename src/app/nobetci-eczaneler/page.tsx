...
export default async function NobetciEczanelerPage() {
  ...
  return (
      ...
      <div className="container mx-auto px-3 sm:px-4 py-6 sm:py-8">
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-600">
            Çanakkale nöbetçi eczaneler listesi. Veriler Çanakkale Eczacı Odası sitesinden otomatik olarak güncellenmektedir.
          </p>
        </div>
        ...
        {eczaneler.length > 0 ? (
          ...
          if (eczane.name && eczane.address) {
            currentEczaneler.push(eczane)
          }
        }
      }
    })
    ...
    // Son ilçeyi kaydet
    if (currentDistrict && currentEczaneler.length > 0) {
      districts.push({
        district: currentDistrict,
        eczaneler: currentEczaneler,
      })
    }
...
export default async function NobetciEczanelerPage() {
  const districtsData = await getNobetciEczaneler()
  const allDistricts = [
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="container mx-auto px-3 sm:px-4 py-6 sm:py-8">
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-600">
            Çanakkale nöbetçi eczaneler listesi. Veriler{' '}
            <a 
              href="https://www.canakkaleeo.org.tr/" 
              target="_blank" 
              rel="noopener noreferrer"
            ...
            {' '}ana sayfasından otomatik olarak güncellenmektedir.
          </p>
        </div>
        ...
        <div className="mb-6 bg-white rounded-lg shadow p-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2">
            {allDistricts.map((district) => {
              const hasData = districtsData.some(d => 
                d.district.toUpperCase().includes(district.toUpperCase()) || 
                district.toUpperCase().includes(d.district.toUpperCase())
              )
              return (
                <a
                  key={district}
                  href={`#${district.toLowerCase().replace(/\//g, '-')}`}
        ...
        {districtsData.length > 0 ? (
          <div className="space-y-6">
            {districtsData.map((districtGroup) => (
              <div
                key={districtGroup.district}
                id={districtGroup.district.toLowerCase().replace(/\//g, '-')}
                className="bg-white rounded-lg shadow overflow-hidden"
              >
                <div className="bg-gradient-to-r from-green-600 to-emerald-600 text-white p-4">
                  <h2 className="text-xl font-bold">{districtGroup.district} NÖBETÇİ ECZANELER</h2>
                </div>
                <div className="p-4 space-y-4">
                  {districtGroup.eczaneler.map((eczane, index) => (
                    <div
                      key={`${eczane.name}-${index}`}
                      className="border-l-4 border-green-500 pl-4 py-3 bg-gray-50 rounded-r-lg"
                    >
                      ...
                      <div className="space-y-1 text-sm text-gray-700">
                        ...
                        {eczane.period && (
                          <div className="flex items-start gap-2 mt-2 pt-2 border-t border-gray-200">
                            <Clock className="h-4 w-4 mt-0.5 text-orange-500 flex-shrink-0" />
                            <span className="flex-1 font-medium text-orange-700">
                              {eczane.period} arasında nöbetçidir.
                            </span>
                          </div>
                        )}
                      </div>
