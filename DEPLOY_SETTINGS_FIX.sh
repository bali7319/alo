#!/bin/bash
# Sunucuya /api/settings endpoint'ini ve cookie domain ayarını kopyala

cd /var/www/alo17

# /api/settings endpoint'ini oluştur
...
# auth.ts dosyasını güncelle - cookie domain ekle
sed -i "s/domain: undefined/domain: process.env.NODE_ENV === 'production' ? '.alo17.tr' : undefined/" src/lib/auth.ts || {
  # Eğer sed başarısız olursa, manuel olarak ekle
  if ! grep -q "domain:.*alo17.tr" src/lib/auth.ts; then
    # Cookie options kısmını bul ve domain ekle
    perl -i -pe "s/(secure: process\.env\.NODE_ENV === 'production',)/\$1\n        domain: process.env.NODE_ENV === 'production' ? '.alo17.tr' : undefined,/" src/lib/auth.ts
  fi
}
