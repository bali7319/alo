#!/bin/bash
# Database index'lerini ekle - Tek komut
ssh root@alo17.tr "sudo -u postgres psql -d alo17_db << 'EOF'
...
CREATE INDEX IF NOT EXISTS idx_listing_expires_at ON \"Listing\"(expiresAt) WHERE \"expiresAt\" > NOW();
